...
public class ChannelControl : UserControl, IViewData
{
	...
	public void InitDgv()
	{
		...
		foreach (DataGridViewColumn column in dgvFreq.Columns)
		{
			column.HeaderCell.Style = style;
		}
		foreach (DataGridViewRow item in (IEnumerable)dgvFreq.Rows)
		{
			item.HeaderCell.Style = style;
		}
	...
	private void cmbTone_Leave(object sender, EventArgs e)
	{
		if (dgvFreq.CurrentCell != null)
		{
			VerifyTone();
			int rowIndex = dgvFreq.CurrentCell.RowIndex;
			if (dgvFreq.CurrentCell.ColumnIndex == 2)
			{
