...
internal class FrequencyHelper
{
    ...
    {
        decimal freqDec = decimal.Parse(freq) * 100000m;
        return Convert.ToInt32(freqDec);
    }

    public static int FreqToBcd(string freq)
    {
        int freqDec = FreqToDec(freq);
        return Common.DecToBcd32(freqDec);
    }
    ...
    public static string BytesToFreq(byte[] data)
    {
        try
        {
            int freqBcd = BitConverter.ToInt32(data, 0);
            int freqDec = Common.BcdToDec32(freqBcd);
    ...
    public static byte[] ToneToBytes(string tone)
    {
        ...
        {
            value = tone.Substring(1, 3);
            short.TryParse(value, NumberStyles.HexNumber, null, out toneShort);
        ...
        else if (tone.IndexOf('.') >= 0)
        {
            ...
            toneShort = (short)Common.MulDecimal(toneDouble, 10.0);
            toneBcd = Common.DecToBcd16(toneShort);
    ...
    public static string BytesToTone(byte[] data)
    {
        ...
        try
        {
            ...
            if (data[0] >= 192)
            {
                toneBcd = (short)((data[0] & 0x3F) * 256 + data[1]);
                return $"D{toneBcd:X3}I";
            }
            if (data[0] >= 128)
            {
                toneBcd = (short)((data[0] & 0x3F) * 256 + data[1]);
                return $"D{toneBcd:X3}N";
            }
            toneBcd = (short)(data[0] * 256 + data[1]);
            toneDec = Common.BcdToDec16(toneBcd);
            return Common.DivDecimal(toneDec, 10).ToString("f1");
        }
        catch (Exception)
        {
            return "";
    ...
    {
        int freqInt = FreqToDec(freq);
        return freqInt % 500 == 0;
    }
}
